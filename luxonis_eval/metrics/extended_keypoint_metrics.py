...
class ExtendedKeypointMetrics(BaseMetric):
    ...
    This metric mirrors the Luxonis Train keypoint metric formulation while
    remaining compatible with ``luxonis-eval`` inputs. In particular it:
    ...
    - applies ``sigmas`` and ``area_factor`` the same way as the train-side
      metric
    - tolerates either normalized or pixel-space keypoints coming from the
      parser, because the generic DepthAI path and the train-style parser path
      package coordinates differently
    """
    ...
    def _update_impl(
        ...
        **kwargs: Any,
        ...
        for det in predictions.detections:
            box = (
                det.getBoundingBox().denormalize(width, height).getOuterXYWH()
            )
            x = float(box[0].x)
            y = float(box[0].y)
            w = float(box[1].width)
        ...
        self.pred_keypoints.append(
            # The generic DepthAI keypoint path packages normalized
            # keypoints, while the luxonis_train postprocessing path keeps
            # post-NMS keypoints in pixels. Normalize that difference here
            # so the metric always evaluates pixel-space coordinates.
            self._denormalize_keypoints_if_needed(
    ...
    @staticmethod
    def _denormalize_keypoints_if_needed(
        keypoints: Tensor, width: int, height: int
    ) -> Tensor:
        """Convert normalized keypoint coordinates to pixels when needed."""
        if keypoints.numel() == 0:
            return keypoints

        x_coords = keypoints[:, 0::3]
        y_coords = keypoints[:, 1::3]
        valid_coords = x_coords.ne(0) | y_coords.ne(0)
        ...
        tolerance = 1e-2
        is_normalized = (
            valid_x.min().item() >= -tolerance
            and valid_y.min().item() >= -tolerance
            and valid_x.max().item() <= 1.0 + tolerance
            and valid_y.max().item() <= 1.0 + tolerance
        )
    ...
    def _get_default_sigmas(self) -> Tensor:
        ...
        if n_keypoints == 17:
            # Standard COCO keypoint OKS sigmas for the 17-keypoint layout.
            return torch.tensor(
                [
                    0.026,
                    0.025,
